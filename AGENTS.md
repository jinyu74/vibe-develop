...
문서에 기능이 누락·모호·충돌하는 경우의 행동 규칙이다. **추측으로 구현하지 않는다.**

#### 불확실성 유형

| 유형 | 설명 | 예시 |
|------|------|------|
| **명세 부재** | 문서에 아예 언급이 없는 기능/동작 | "비밀번호 재설정 시 이메일 발송 여부가 01에 없음" |
| **명세 모호** | 문서에 있지만 해석이 여러 가지 가능 | "대량 데이터 처리" — 기준이 100건인지 10만건인지 불명확 |
| **문서 간 충돌** | 두 문서가 서로 다른 내용을 기술 | 01에는 "이메일 필수"인데 05의 API에 email이 optional |
| **기술적 미결정** | 구현에 필요한 기술 선택이 문서에 없음 | "캐시 전략이 08에 정의되지 않음" |
| **외부 의존 미정의** | 연동할 외부 시스템의 스펙이 없음 | "결제 PG사 API 스펙이 05에 미등록" |

#### 심각도별 대응 행동

| 심각도 | 판단 기준 | AI 행동 |
|--------|----------|---------|
| **차단(Block)** | 잘못 구현 시 데이터 손실, 보안 문제, 과금 오류 가능 | 즉시 중단 → 사용자에게 결정 요청, 코드 작성 안 함 |
| **확인(Confirm)** | 두 가지 이상 합리적 해석이 가능 | 선택지를 제시 → 사용자 선택 후 구현 |
| **기록(Record)** | 한 가지 합리적 기본값이 존재 | 기본값으로 구현 + 가정 사항을 명시적으로 기록 |

#### 보고 형식

불확실성 발견 시 아래 형식으로 보고한다:
...
| # | 유형 | 위치 | 내용 | 심각도 | 제안 |
|---|------|------|------|--------|------|
| 1 | 문서 간 충돌 | 01 F003 ↔ 05 POST /users | email 필수 여부 불일치 | 차단 | 팀 결정 필요 |
| 2 | 명세 모호 | 01 F005 | "대량" 기준 미정의 | 확인 | A: 1000건 / B: 10000건 |
| 3 | 기술적 미결정 | 08 캐시 | 캐시 TTL 미정의 | 기록 | 300초 기본값 적용 |
```

#### 가정 사항 추적

"기록(Record)" 심각도로 AI가 가정하고 진행한 항목은 `CONTEXT.md`의 가정 사항 섹션에 남긴다:

```markdown
## 가정 사항 (AI 판단)

| # | 날짜 | 기능 | 가정 내용 | 근거 | 확인 상태 |
|---|------|------|----------|------|----------|
| A001 | 2026-02-19 | F003 캐시 | TTL 300초 적용 | 일반적 API 캐시 기본값 | 미확인 |
```

#### 에스컬레이션 규칙

- **차단** 항목이 1개라도 있으면 → 해당 기능 전체 구현을 보류
- **확인** 항목이 3개 이상 누적되면 → 문서 보완을 먼저 권고
- 같은 문서에서 **충돌**이 반복 발견되면 → 해당 문서의 전체 리뷰를 권고
...
버전이 올라갈 때(v0.0.1 → v0.0.2), 새 버전 문서에서 이전 버전과 달라진 항목을 태그로 표시한다.
AI는 이 태그를 보고 신규/변경/삭제 대상을 정확히 파악한다.

### 태그 종류

| 태그 | 의미 | 사용 위치 |
|------|------|----------|
| `[추가]` | 이전 버전에 없던 새 항목 | 기능, 화면, API, 테스트 |
| `[변경]` | 이전 버전에서 수정된 항목 | 기능, 화면, API, 테스트 |
| `[삭제]` | 이전 버전에서 제거된 항목 | 기능, 화면, API, 테스트 |
| `[이동]` | 다른 서비스/모듈로 이동된 항목 | 기능, API |

### 사용 예시

```markdown
# 01-feature-list.md (v0.0.2)

| F-ID | 기능명 | 태그 | 설명 |
|------|--------|------|------|
| F-FW-001 | SFTP 감시 | [변경] | 양방향 상태 표시 추가 |
| F-FW-004 | 로컬 폴더 감시 | [추가] | 신규 기능 |
| F-FW-002 | 수동 동기화 | [삭제] | 자동화로 대체 |
```

### 규칙

1. **첫 버전(v0.0.1)에는 태그 사용하지 않음** — 모든 것이 신규이므로
2. **v0.0.2부터 이전 버전 대비 변경 사항에 태그 부착**
3. `[변경]` 태그에는 **무엇이 바뀌었는지** 간략히 기재
4. `[삭제]` 태그에는 **삭제 이유** 기재
5. CONTEXT.md의 "변경 범위" 섹션과 문서 내 태그가 일치해야 함
