...
> **상태 값**: `계획` → `개발중` → `릴리스` → `폐기`
> 핫픽스는 `v0.0.1-hotfix.1` 형태로 버전 이력에 추가.

---

## 변경 범위 (현재 버전)

> 현재 작업 중인 버전에서 변경되는 항목을 명시합니다.
> 이전 버전에서 새 버전으로 올릴 때만 작성합니다 (v0.0.1 → v0.0.2 등).
...
> 프로덕션 긴급 수정 이력. 상세는 `docs/{service}/v{version}/HOTFIX.md` 참조.
...
<!-- AI가 불확실성 대응 프로토콜의 "기록(Record)" 심각도로 가정하고 진행한 항목을 기록한다. 팀이 정기적으로 확인하여 승인/수정한다. -->
